...
class _JoinSpec:
    ...
    def __init__(self, spec):
        if (len(spec) != 2 or not isinstance(spec[0], str) or
                ...
                not isinstance(spec[1][0][0], str) or
                not isinstance(spec[1][0][1], Dataset) or
                len(spec[1][1]) != 2 or not callable(spec[1][1][1])):
...
class SparkEngineImpl(nudl.dataset.DatasetEngine):
    ...
    def read_csv(self, seed: typing.Any, file_spec: str) -> Dataset:
        # TODO(catalin): Get the schema for seed etc, add options and such.
        return Dataset(self, seed, self.session.read.csv(file_spec))

    def read_parquet(self, seed: typing.Any, file_spec: str) -> Dataset:
        # Get the schema for seed etc, add options and such.
        return Dataset(self, seed, self.session.read.parquet(file_spec))
    ...
    def dataset_filter(self, src: Dataset,
        ...
        return Dataset(self, src.seed, src.df.rdd.filter(f).toDF())
    ...
    def dataset_map(self, src: Dataset,
        ...
        return Dataset(self, src.seed, src.df.rdd.map(f, True).toDF())
    ...
    def dataset_flat_map(self, src: Dataset,
        ...
        self._check_my_dataset(src)
        # For now preserve partitioning..
        return Dataset(self, src.seed, src.df.rdd.flatMap(f, True).toDF())
    ...
    def dataset_aggregate(self, src: Dataset,
        ...
        return Dataset(
            self, mapper(src.seed),
            src.df.rdd.map(mapper).toDF().groupBy(group_names).agg(*agg_spec))
    ...
    def dataset_join_left(self, src: Dataset,
        ...
        for right in right_spec:
            ...
            if spec.join_spec == "right_multi":
                ...
                builder = (lambda t, field_name=spec.field_name, builder=
                           builder: _join_builder_next(t, field_name, builder))
        ...
        return Dataset(self, pyspark.sql.Row(**joint_seed),
